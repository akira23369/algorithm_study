...
void heap_sort(T arr[], int n)
{
    ...
    heap<T> h = heap<T>(arr, n);
    for (int i = n - 1; i >= 0; i--) arr[i] = h.extract();
}

int main()
{
    heap<int> m_heap = heap<int>(100);
    std::cout << m_heap.size() << "\n";
    srand(time(NULL));
    for (int i = 0; i < 10; i++)
        m_heap.insert(rand() % 100);
    // m_heap.testPrint();
    // for (int i = 0; i < 4; i++) std::cout << m_heap.extract() << "\n";
    // m_heap.testPrint();
    int n = 10;
